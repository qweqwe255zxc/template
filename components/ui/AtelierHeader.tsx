  ...
  const textBlock = (
    <div className={cn("min-w-0", centered && "flex flex-col items-center")}>
      ...
      {/* Штрих. Размер задан константами, и это не нарушение §1.5 п. 3:
          ...
          Цвет — `bg-accent-border`, а не `bg-accent`, и это важно.
          `--accent-border` для того и заведён: на бумаге и на белом листе
          он равен самому акценту, на акцентной заливке становится
          светлым (акцент по акценту не виден вовсе), на тёмном блоке —
          приглушённо-светлым. То есть штрих остаётся видимым на всех
          четырёх поверхностях без единого условия в коде. С `bg-accent`
          он молча исчезал бы в CTA этого же семейства, где заливка
          акцентная по умолчанию.

          aria-hidden не нужен: у пустого div нет ни роли, ни имени, и
          скринридер его не озвучивает. */}
      {title ? (
        <div
          className={cn("mt-6 h-0.5 w-14 bg-accent-border", centered && "mx-auto")}
          data-reveal
        />
      ) : null}
