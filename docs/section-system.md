...
> **Демо-конфиг — стенд каталога, а не образец для проекта.**
> `content/site.config.ts` нарочно ставит у каждой секции свой,
> максимально непохожий на соседей `variant` (`service` → `plain` →
> `bento` → `panel` → `cards` → `photo-grid` → `spotlight` → …): так за
> одну прокрутку видно разные раскладки. Копировать этот принцип в
> клиентский проект нельзя.
>
> **Реальный сайт держится на 2–3 сквозных приёмах, а не на максимальном
> разнообразии.** Приём — это повторяющееся решение: карточные раскладки
> везде плюс один тёмный блок и один акцентный; или семейство
> `sticky-split` на всех секциях (CLAUDE.md §2.15 — «приём обязывает»:
> либо весь сайт, либо ни одной секции); или строгий табличный «Эконом» с
> единственным исключением на кейсах. Страница, где у каждой секции своя
> «самая интересная» раскладка, читается как демонстрация возможностей
> шаблона, а не как сайт клиента, — и это ровно тот же класс ошибки, что
> дефолтная палитра: разнообразие ради разнообразия так же безлико, как
> его отсутствие.
>
> Практическое правило подбора: выбери 2–3 приёма ПОД БРЕНД (см. §5
> CLAUDE.md, шаг 0 — референсы), а дальше отклоняйся от них только там,
> где раскладка не тянет содержимое: длинный перечень, сложная тарифная
> сетка, кейсы с фото. Ритм при этом даёт чередование поверхностей
> (CLAUDE.md §3), а не смена раскладок.
...
| `editorial` | семейство §editorial: колонтитул на линейке, заголовок-афиша в верхнем регистре (потолок кегля 104px), лид и кнопки строкой, `image` полосой во всю ширину окна | — (`image` не обязателен, `widget` не читается) |
...
| `editorial` | семейство §editorial: полоса-таблица без карточек — СПЛОШНАЯ линейка (`border-rule-strong`) над каждой ячейкой, волосяная между колонками, поднятый до 76px потолок `--size-stat` в поддереве полосы. Не читает `containerVariant`, `fillLastRow`, `image` | `icon` не читает; `text` — абзац под подписью |
...
| `editorial` | семейство §editorial: нумерованный перечень, линию держит `border-top` каждой ячейки, колонки разделяет только воздух; `item.link` строкой внизу ячейки на `mt-auto` | форсирует `cards` |

**Если у хотя бы одного `item` задан `photo`, а `variant` — `table`,
`sticky-split`, `compact` или `editorial`** — роутер форсирует `variant="cards"`
...
| `editorial` | семейство §editorial: шаги в ряд на СПЛОШНОЙ линейке (`border-rule-strong`), номер и `meta` одной строкой над заголовком. Колонок — по числу шагов: до 3 на `lg`, до 5 на `xl`. `icon` — голый глиф, `iconShape` не читает |
...
| `editorial` | семейство §editorial: сетка фотографий без карточек, подпись лежит на поверхности секции (название и год строкой по базовой линии, категория капителью). `align: "center"` не читает |
...
| `editorial` | семейство §editorial: цитаты столбцами прямо на поверхности, кегль `text-quote`, подпись капителью на волосяной линейке прижата `mt-auto` — линейки всего ряда выстраиваются в одну горизонталь. Не читает `rating`, `photo`, `result`, `featured`, `fillLastRow`; `trust` читает |
...
| `editorial` | семейство §editorial: портреты 3:4 без карточек, над каждым — волосяная линейка с индексом («01»). Линейка обязана быть с индексом: пустую над фото уже снимали в `columns` как «обрывок чужой таблицы». Стаж прижат `mt-auto`, `link` — тихой ссылкой капителью. Не читает `social`, `tags`, `image`, `fillLastRow` |
...
| `highlights?: AboutHighlight[]` | нет | — | ряд иконка+заголовок+текст под фото; читают `panel` и `editorial` (последний — врезками в 2 колонки на волосяных линейках, без иконки) |
...
| `editorial` | семейство §editorial: ось 7/5, раздел открывается не заголовком, а первым абзацем `text` ступенью `text-quote`; `title` уходит в колонтитул рядом с `eyebrow`. Ниже — `highlights` врезками и кнопки, последний из этих блоков прижат `lg:mt-auto` к низу ряда (высоту задаёт фото 3:4, иначе под кнопками остаётся дыра). `photo` обязателен. Не читает `aside`, `panel`, `photoCaption`, `badge`, `decorative`, `frame` |
...
| `support?: FaqSupport` | нет | — | карточка «Остались вопросы?»; читают все варианты |
...
| `editorial` | семейство §editorial: колонтитул и заголовок капслоком слева, аккордеон справа. Ось 5/7, а не 4/8 как у `split-sidebar`: капслок при равном кегле шире примерно на четверть, и в колонке 4/12 заголовок рвался бы на слог. Заголовок НЕ залипает — залипание опознавательный знак `sticky-split` |
...
| `editorial` | семейство §editorial: карточки, шапка которых набрана как колонтитул раздела — позиционный индекс капителью слева, `plan.badge` плашкой справа. Цена `text-h2`, чек-лист тире |
...
| `editorial` | семейство §editorial: тёмная полоса (`surface` по умолчанию `ink`, не `accent`), заголовок `text-h1` в верхнем регистре 7/12 слева, лид с кнопками 5/12 справа, обе колонки по нижнему краю. `eyebrow` — колонтитул на линейке. От `theme.titleStyle` не зависит |
...
| `layout: "plain"\|"cardContainer"` | нет | `"plain"` | подложка под самой формой (не колонки); `cardContainer` — `<form>` в `Card variant="elevated"`. Читают все варианты, включая `editorial` |
...
| `editorial` | семейство §editorial: те же 5/7, что у `split`, но реквизиты без иконок и без линеек между строками — подпись капителью, значение `font-display text-h3` (`ContactDetails layout="editorial"`). Число колонок реквизитов решает контейнерный запрос, а не брейкпоинт окна |
...
| `editorial` | семейство §editorial: вордмарк и навигация капителью с разрядкой (`tracking-[0.14em]`), активный пункт только цветом. Единственный вариант, непрозрачный с первого кадра — `transparentBeforeScroll` игнорируется, потому что волосяная линейка под баром часть приёма |
...
`columns?` читают только `bold`/`classic`/`monogram`/`editorial`
(колончатые раскладки); `social?`/`newsletter?` читают **все восемь** вариантов —
...
| `editorial` | семейство §editorial: два яруса на линейках — СПЛОШНАЯ сверху (закрывает страницу тем же весом, что полоса цифр и ряд шагов), волосяная между ярусами. Служебный текст капителью с разрядкой, `social` выводится ТЕКСТОМ по `label`, а не иконками | под `columns`, на волосяной линейке |
...
Текущее состояние репозитория — демо-сборка «Модуль» (SaaS-конструктор):
акцент-петроль `#16494f` по тёплой бумаге `#f4f2ee`, пара
Wix Madefor Display / Wix Madefor Text + Golos Text, `preset: "standard"`.
Это пример, а не база для копирования: палитра и шрифты нового проекта
собираются по референсам клиента (CLAUDE.md §5, шаг 0), а состав
`variant` — по правилу «2–3 сквозных приёма» из врезки в начале файла.
...
Один и тот же `variant` в двенадцати секциях: Hero, Stats, Features,
...
Второе сквозное семейство после `sticky-split` — перенос лендинга
Sirotov Architects. Закрыто целиком: `variant: "editorial"` есть у
**всех двенадцати** секций конфига и у обоих блоков вне его
(`header.variant`, `footer.variant`). Сайт собирается этим приёмом без
примеси карточных раскладок.

Смотреть семейство целиком — `/qa-audit/editorial`: там вся страница
(хедер, двенадцать секций, футер) собрана одним приёмом. Постраничные
стенды показывают вариант рядом с восемью чужими раскладками, и на них
принципиально не видно того, ради чего сквозное семейство и заводят:
держится ли ритм, когда одинаковые колонтитулы идут подряд, и не
сливаются ли соседние разделы в одну простыню.

Приём: страница держится не на карточках и заливках, а на **линиях и
регистре**.

- Шапка раздела — `components/ui/EditorialHeader.tsx`: линейка во всю
  ширину колонки, на ней колонтитул (`number` слева, `eyebrow` справа),
  под ним заголовок в ВЕРХНЕМ регистре узкой мерой, дальше лид. Шапка
  живёт одним компонентом ровно по той же причине, что `StickySplit`:
  колонтитул повторяется на странице десять-двенадцать раз подряд, и
  разъехавшийся на пиксель отступ превращает приём в случайность.
- Толщина линии несёт смысл: волосяная (`border-rule`) — перечень
  (Features), сплошная (`border-rule-strong`) — последовательность
  (Steps). Это единственный способ различить их, не заводя карточек.
- Кегль растёт от раздела к странице: `.section-title-scale` у шапок
  разделов, `text-h1` у CTA, поднятый до 104px потолок `EDITORIAL_H1`
  у Hero (`Hero/parts/headlineScale.ts`).
- Монохром — свойство ПАЛИТРЫ проекта, а не варианта. Ни один вариант
  семейства не отменяет акцент шаблона: последняя строка `headline`
  в Hero остаётся акцентной, `primary`-кнопка — акцентной заливкой. У
  монохромного бренда акцент придёт из `theme/tokens.css` почти чёрным
  сам.

Ограничения, которые нужно знать до сборки:

- **Фото в `Features/editorial` нет.** Линию ряда держит `border-top`
  ячейки, и картинка произвольной высоты её рвёт — роутер форсирует
  `cards` (раздел 2, тот же список, что `table`/`compact`).
- **Hero/editorial не читает `widget`** — второй колонки в раскладке
  нет вовсе; `image` идёт полосой во всю ширину окна и НЕ обязателен.
- **CTA/editorial по умолчанию `ink`, а не `accent`.** Если проекту
  нужен акцентный финал — `surface: "accent"` в конфиге.
- **`About/editorial` требует `photo`** — без него от раскладки
  остаётся одна текстовая колонка 7/12 с пустой половиной справа, и
  роутер откатывается на `type-only` с предупреждением в dev.
- **`FAQ/editorial` заголовок НЕ залипает**, хотя в исходном приёме он
  был `sticky`. Залипание — опознавательный знак `sticky-split`; нужен
  именно он, у той же секции есть `variant: "sticky-split"`.
- **`Header/editorial` игнорирует `transparentBeforeScroll`** —
  волосяная линейка под баром часть приёма, а прозрачный хедер её
  показать не может. Единственный вариант хедера, непрозрачный с
  первого кадра.
- **Иконок в семействе нет нигде**, где они были декором: `Stats`
  не читает `item.icon`, `Team` — `social`, `ContactForm` рисует
  реквизиты без пиктограмм (`ContactDetails layout="editorial"`),
  `Footer` выводит соцссылки текстом. Исключение — `Steps`, где
  `item.icon` рендерится голым глифом без плашки.
...
В `index.tsx` каждой секции стоит комментарий «ТАРИФНАЯ ПОМЕТКА»:
весь каталог вариантов, существовавший до `editorial`, помечен как
**эконом-класс**. Пометка НАМЕРЕННО отделена от тарифной механики
шаблона — `theme.preset`, `PRESET_DEFAULTS` (`lib/preset.ts`) и блоки
