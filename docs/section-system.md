...
Восемь вариантов (считая `sticky-split`). `band`/`grid` — плоские, без заголовка секции (только
...
Шесть вариантов (считая `sticky-split`). `quotes`/`cards` — заголовок через `SectionHeader`.
...
Восемь вариантов (считая `sticky-split`). `columns`/`rows`/`cards` — заголовок через
...
Шесть вариантов (считая `sticky-split`). `photo` — исходный, через `parts/AboutLayout.tsx`, `photo`
...
Пять вариантов (считая `sticky-split`). `narrow`/`wide` делят общий `parts/FaqBody.tsx` (шапка +
...
Sirotov Architects. Пока сделаны **шесть** секций из двенадцати: Hero,
Features, Steps, Gallery, Pricing, CTA. Остальные шесть (Stats, About,
Testimonials, Team, FAQ, ContactForm) и блоки вне конфига
(Header/Footer) этого варианта ещё не имеют — в их `index.tsx` стоит
пометка об этом.
...
- Шапка раздела — `components/ui/EditorialHeader.tsx`: линейка во всю
  ...
  колонтитул повторяется на странице шесть-девять раз подряд, и
...
- **Приём обязывает** ровно так же, как `sticky-split`: одинокая
  editorial-секция среди карточных читается как сбой. Пока семейство
  неполное, это значит, что чистого «editorial-сайта» из шаблона
  собрать нельзя — шесть секций из двенадцати.
