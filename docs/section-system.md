...
`parts/FeaturesHeader.tsx`. `split-list` собирает шапку сам: она уезжает
...
| `split-list` | заголовок в колонке 4/12 слева (`lg:sticky` под хедером), список строками на линейках справа. Для длинного перечня 6–10 пунктов | форсирует `cards` |
...
`split-list` или `compact`** — роутер форсирует `variant="cards"`
