...
export interface SectionBase {
  ...
  /**
   * Бегущая строка у НИЖНЕГО края секции — полоса во всю ширину окна с
   * повторяющейся фразой капслоком («Доставим за 40 минут!»).
   *
   * Читают только варианты семейства `market` (CLAUDE.md §2.19): это его
   * второй несущий приём после кричащего заголовка, и в чужой раскладке
   * полоса читалась бы как баннер, приехавший из другого макета.
   * Остальные одиннадцать вариантов каждой секции поле игнорируют.
   *
   * Нижний край, а не верхний, — чтобы полоса читалась как отбивка ПОСЛЕ
   * раздела: `hero.ticker` даёт полосу под первым экраном, `steps.ticker`
   * — после «как работаем», `contact.ticker` — перед подвалом. Ровно три
   * полосы на страницу, как в исходном приёме; поставить её всем
   * двенадцати разделам технически можно, но тогда приём перестаёт быть
   * акцентом и превращается в фон.
   *
   * Направления у поля нет намеренно. В исходнике соседние полосы бегут
   * в разные стороны, но секция не знает своего номера на странице, а
   * второе поле ради одного этого — мусор в конфиге. Все полосы едут
   * влево; направление осталось пропом самого `components/ui/Ticker.tsx`
   * для ручной сборки.
   */
  ticker?: string;
}
...
export interface HeroSection extends SectionBase {
  ...
  variant?:
    ...
    | "atelier"
    /**
     * Семейство market: заголовок капслоком слева (1.15fr), лид и кнопки
     * справа (1fr) по нижнему краю, под ними строка «часы / адрес / телефон»
     * из `facts`. `widget` не читается, `image` не обязателен — первый экран
     * этого приёма чисто типографский.
     */
    | "market";
  /**
...
export interface StatsSection extends SectionBase {
  ...
  variant?:
    ...
    | "atelier"
    /**
     * Семейство market: четыре колонки без карточек, рамок и иконок —
     * гигантское акцентное число и мелкая приглушённая подпись под ним.
     * `icon`, `containerVariant`, `image` и `fillLastRow` не читаются.
     */
    | "market";
  /**
...
export interface FeaturesSection extends SectionBase {
  ...
  variant?:
    ...
    | "atelier"
    /**
     * Семейство market: сетка плоских карточек, акцентная иконка голым
     * глифом НАД заголовком (у `cards` она в плашке в строке с ним, у
     * `product` — плашкой над). `photo` не поддерживает — роутер уводит в
     * `cards`.
     */
    | "market";
  columns?: 2 | 3;
...
export interface StepsSection extends SectionBase {
  ...
  variant?:
    ...
    | "atelier"
    /**
     * Семейство market: четыре колонки, крупный акцентный номер над
     * заголовком. Без карточек, без оси и без соединительной линии —
     * шаг держит только кегль номера.
     */
    | "market";
...
export interface GallerySection extends SectionBase {
  ...
  variant?:
    ...
    | "atelier"
    /**
     * Семейство market: кадры 4:3, крупная подпись капслоком ВНУТРИ кадра
     * по нижнему краю на градиентной подложке, под кадром заголовок и
     * текст. Единственная галерея шаблона с текстом внутри кадра.
     * Подпись в кадре берётся из ПЕРВОЙ пары `item.stats` (значение
     * крупно, подпись под ним); у кейса без `stats` кадр остаётся чистым,
     * без подложки. На `category` она не откатывается — та уже напечатана
     * строкой под кадром.
     */
    | "market";
  items: CaseItem[];
...
export interface TestimonialsSection extends SectionBase {
  ...
  variant?:
    ...
    | "atelier"
    /**
     * Семейство market: плоские карточки с крупной акцентной кавычкой
     * над цитатой, автор прижат к низу. Ни аватара, ни рейтинга.
     */
    | "market";
...
export interface TeamSection extends SectionBase {
  ...
  variant?:
    ...
    | "atelier"
    /**
     * Семейство market: портреты 3:4 без карточек, имя полужирным,
     * роль обычной приглушённой строкой. Отличие от `atelier`, где
     * портреты того же формата: там роль набрана капителью с разрядкой.
     */
    | "market";
  items: TeamMember[];
...
export interface AboutSection extends SectionBase {
  ...
  variant?:
    ...
    | "atelier"
    /**
     * Семейство market: фото 4:5 слева, справа шапка по левому краю (без
     * шеврона), текст и ряд мини-цифр из `panel.stats`. `photo` НЕ
     * обязателен — без него раздел честно сворачивается в одну колонку.
     */
    | "market";
...
export interface FaqSection extends SectionBase {
  ...
  variant?:
    ...
    | "atelier"
    /**
     * Семейство market: центрированная шапка с шевроном, аккордеон
     * плоскими карточками в узкой колонке. Не залипает.
     */
    | "market";
  items: FaqItem[];
...
export interface PricingSection extends SectionBase {
  ...
  variant?:
    ...
    | "atelier"
    /**
     * Семейство market: плоские карточки с перевёрнутой иерархией —
     * название мелкой капителью, цена крупно; выделенный тариф сплошной
     * акцентной заливкой, чек-лист БЕЗ маркеров.
     */
    | "market";
  items: PricingPlan[];
  note?: string;
...
export interface CtaSection extends SectionBase {
  ...
  variant?:
    ...
    | "atelier"
    /**
     * Семейство market: акцентная полоса — гигантский заголовок капслоком
     * слева, лид и светлая кнопка справа.
     */
    | "market";
  actions?: CtaLink[];
  note?: string;
...
export interface ContactSection extends SectionBase {
  ...
  variant?:
    ...
    | "atelier"
    /**
     * Семейство market: реквизиты слева (5/12) парами «подпись капслоком —
     * значение», без пиктограмм; форма справа (7/12), карта полосой под
     * ними.
     */
    | "market";
  /**
...
export type HeaderVariant =
  ...
  | "atelier"
  /**
   * Семейство market: вордмарк капслоком слева, навигация по центру
   * АКЦЕНТНЫМ цветом, кнопка справа. Единственный хедер шаблона с
   * цветной навигацией.
   */
  | "market";
...
export type FooterVariant =
  ...
  | "atelier"
  /**
   * Семейство market: тёмный подвал одной строкой — вордмарк слева,
   * навигация по центру, справа часы акцентом, адрес и телефон.
   * Второй после `atelier` футер, выводящий `contacts`.
   */
  | "market";
